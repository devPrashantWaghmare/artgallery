...
const ManageRoles = () => {
    ...
    const fetchRoles = async () => {
        try {
            setLoading(true);
            const [creatorsResponse, subadminsResponse] = await Promise.all([
                apiClient.get('/api/roles/filter?role=SuperAdmin')
                ,
                apiClient.get('/api/roles/filter?role=SubAdmin')]);
            setSubadmins(subadminsResponse.data);
        } catch (err) {
            console.error('Error fetching roles:', err);
            setError('Failed to fetch roles. Please try again.');
    ...
    const fetchSubadminCreators = async (subadminId) => {
        setLoading(true);
        try {
            const response = await apiClient.get(`/api/admin/roles/subadmin-creators/${subadminId}`);
            setSubadminCreators(response.data);
        } catch (err) {
            console.error('Error fetching subadmin creators:', err);
            setError('Failed to fetch creators for this subadmin.');
...
const ManageRoles = () => {
  ...
  const handleAssignSubadmin = async () => {
    ...
    try {
      setLoading(true);
      await apiClient.post('/api/admin/roles/assign-subadmin', {
        subadminId: selectedSubadmin,
        contentCreatorIds: selectedCreators,
      });
      setSelectedCreators([]);
  ...
  return (
    <div className="manage-roles-container">
      ...
      <div className="assign-section">
        ...
        <button className="assign-btn" onClick={handleAssignSubadmin} disabled={loading}>
          {loading ? 'Assigning...' : 'Assign'}
        </button>
      </div>
      ...
      <div className="view-section">
        ...
        <div className="form-group">
          <label htmlFor="subadmin-select-view">Select Subadmin:</label>
          <select id="subadmin-select-view" onChange={(e) => fetchSubadminCreators(e.target.value)}>
            <option value="">-- Select Subadmin --</option>
            {subadmins.map((subadmin) => (
              <option key={subadmin._id} value={subadmin._id}>
                {subadmin.name}
        ...
        {loading ? (
          <p>Loading...</p>
        ) : subadminCreators.length === 0 ? (
          <p>No creators assigned to this subadmin.</p>
        ) : (
          <table className="creators-table">
            <thead>
              <tr>
